...
#[derive(Deserialize)]
pub struct MapPoolsParameters {
    pool_manager: String,
    hooks_blacklist: Vec<String>,
}
...
pub fn map_pools_created(
    ...
    let params =
        serde_qs::from_str::<MapPoolsParameters>(&params).expect("parameter parse failure");
...
fn get_new_pools(
    ...
    params: MapPoolsParameters,
) {
    let blacklist: HashSet<Vec<u8>> = params
        .hooks_blacklist
        .iter()
        .map(|addr| {
            let addr = addr.strip_prefix("0x").unwrap_or(addr);
            hex::decode(addr).expect("blacklist entry parse failure")
        })
        .collect();

    let pool_manager_address = hex::decode(
        params
            .pool_manager
            .strip_prefix("0x")
            .unwrap_or(&params.pool_manager),
    )
    .unwrap();
    ...
    let mut on_pool_created = |event: Initialize, _tx: &eth::TransactionTrace, _log: &eth::Log| {
        ...
        if !blacklist.contains(&event.hooks) {
            new_pools.push(TransactionEntityChanges {
                ...
                entity_changes: vec![EntityChanges {
                    ...
                    attributes: vec![
                        Attribute {
                            ...
                            value: pool_manager_address.clone(),
                ...
                component_changes: vec![ProtocolComponent {
                    ...
                    static_att: vec![
                        ...
                        // Static fees are represented in hundredths of a
                        // bip, can be set to a value between 0 and 1000000
                        // (100%) and are immutable. If the value is set to 0x800000 then
                        // the pool is flagged as using dynamic fees. The dynamic fees changes
                        // should be tracked in state attributes and the
                        // flag value set here should remain untouched (it
                        // is needed for generating the PoolKey for
    ...
    eh.filter_by_address(vec![Address::from_slice(pool_manager_address.as_slice())]);
