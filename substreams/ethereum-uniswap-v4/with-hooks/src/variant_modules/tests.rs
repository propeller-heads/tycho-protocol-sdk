#[cfg(test)]
mod tests {
    use substreams_ethereum::pb::eth::v2::{Block, Log, TransactionTrace, TransactionReceipt};
    ...
    fn create_mock_pools_created() -> BlockEntityChanges {
        ...
        component_change.id = "0x85405f10672f18aa00705afe87ec937d4eadcfc2652f223591b17040ea1d39d4".to_string();
    ...
    fn test_track_uniswap_pools_by_hook() {
        ...
        let result = crate::variant_modules::store_pools_per_hook::_track_uniswap_pools_by_hook(pools_created);
    ...
    fn test_basic_functionality() {
        ...
        let hooks_attr = component_change.static_att.iter()
            .find(|attr| attr.name == "hooks");
        
        assert!(hooks_attr.is_some());
        assert_eq!(hooks_attr.unwrap().value, hex::decode("D585c8Baa6c0099d2cc59a5a089B8366Cb3ea8A8").unwrap());
        
        println!("Successfully extracted hook address from component change");
    }

    // Test based on real block 23120299 and transaction 0xb2347c7bd922fe5c7f5027523e3f3b4c2e72e7b535e4d0ddd2f4ea4f21c6edbf
    fn create_real_block_23120299() -> Block {
        ...
        tx.hash = hex::decode("b2347c7bd922fe5c7f5027523e3f3b4c2e72e7b535e4d0ddd2f4ea4f21c6edbf").unwrap();
        ...
        pool_deployed_log.address = hex::decode("000AFbF798467f9b3b97F90D05Bf7Df592d89A6CF0").unwrap();
        
        // Real addresses from the transaction
        pool_deployed_log.topics = vec![
            // PoolDeployed event signature
            hex::decode("5f7560a5797edc6f72421362defa094d690eb9f7ced3cc5a5c13383502e4fcc5").unwrap(),
            // asset0: USDC
            hex::decode("000000000000000000000000a0b86991c6218b36c1d19d4a2e9eb0ce3606eb48").unwrap(),
            // asset1: USDT  
            hex::decode("000000000000000000000000dac17f958d2ee523a2206206994597c13d831ec7").unwrap(),
            // eulerAccount  
            hex::decode("0000000000000000000000000AFbF798467f9b3b97F90d05bF7df592D89A6CF6").unwrap(),
        ];
        
        // Pool address in data field
        pool_deployed_log.data = hex::decode("000000000000000000000000D585c8Baa6c0099d2cc59a5a089B8366Cb3ea8A8").unwrap();
        ...
        initialize_log.topics = vec![
            ...
            hex::decode("dd466e674ea557f56295e2d0218a125ea4b4f0f6f3307b95f85e6110838d6438").unwrap(),
        ];
    ...
    fn test_handle_pool_uninstalled_single_pool() {
        ...
        assert_eq!(pool_ids[0], "0x85405f10672f18aa00705afe87ec937d4eadcfc2652f223591b17040ea1d39d4");
    ...
    fn test_handle_pool_uninstalled_multiple_pools() {
        ...
        store.insert(
            "hook:0xaabbccdd".to_string(),
            "0xpool1;0xpool2;0xpool3;".to_string(),
        );
