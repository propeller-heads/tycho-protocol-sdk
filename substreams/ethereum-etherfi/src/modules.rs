...
pub fn map_relative_balances(
    ...
    let balance_deltas = block
        ...
        .flat_map(|log| {
            ...
            if let Some(event) = abi::dmmpool_contract::events::Mint::match_and_decode(log) {
            ...
            } else if let Some(event) = abi::dmmpool_contract::events::Burn::match_and_decode(log) {
                let component_id = address_to_hex(log.address());
            ...
            } else if let Some(event) = abi::dmmpool_contract::events::Swap::match_and_decode(log) {
                let component_id = address_to_hex(log.address());
