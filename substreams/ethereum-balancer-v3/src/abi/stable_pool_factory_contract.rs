...
pub mod functions {
    ...
    impl Create {
        ...
        pub fn encode(&self) -> Vec<u8> {
            let data = ethabi::encode(&[
                ...
                ethabi::Token::Tuple(vec![
                    ethabi::Token::Address(ethabi::Address::from_slice(&self.role_accounts.0)),
                    ethabi::Token::Address(ethabi::Address::from_slice(&self.role_accounts.1)),
                    ethabi::Token::Address(ethabi::Address::from_slice(&self.role_accounts.2)),
                ]),
                ethabi::Token::Uint(ethabi::Uint::from_big_endian(
                    match self
                        .swap_fee_percentage
                ...
                ethabi::Token::Address(ethabi::Address::from_slice(&self.pool_hooks_contract)),
                ethabi::Token::Bool(self.enable_donation.clone()),
                ethabi::Token::Bool(
                    self.disable_unbalanced_liquidity
                        .clone(),
                ),
    ...
    impl substreams_ethereum::rpc::RPCDecodable<Vec<u8>> for GetDefaultPoolHooksContract {
        fn output(data: &[u8]) -> Result<Vec<u8>, String> {
            Self::output(data)
        }
    }
    ...
    impl substreams_ethereum::rpc::RPCDecodable<substreams::scalar::BigInt> for GetPauseWindowDuration {
        fn output(data: &[u8]) -> Result<substreams::scalar::BigInt, String> {
            Self::output(data)
        }
    }
    ...
    impl substreams_ethereum::rpc::RPCDecodable<substreams::scalar::BigInt> for GetPoolCount {
        fn output(data: &[u8]) -> Result<substreams::scalar::BigInt, String> {
            Self::output(data)
        }
    }
