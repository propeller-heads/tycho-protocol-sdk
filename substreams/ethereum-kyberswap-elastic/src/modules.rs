    ...
    let balance_deltas = block
        ...
        .flat_map(|log| {
            ...
            if let Some(event) = abi::pool_contract::events::Mint::match_and_decode(log) {
                // Mint event: (reserve0, reserve1) += (qty0, qty1)
                ...
                if let Some((token0, token1)) = maybe_get_pool_tokens(&store, &component_id) {
                    deltas.extend_from_slice(&[
                        BalanceDelta {
                            ...
                            token: token0,
                            delta: event.qty0.to_signed_bytes_be(),
                        ...
                        BalanceDelta {
                            ...
                            tx: Some(log.receipt.transaction.into()),
                            token: token1,
                            delta: event.qty1.to_signed_bytes_be(),
                            component_id: string_to_bytes(&component_id),
                        },
                    ]);
                }
            } else if let Some(event) = abi::pool_contract::events::Burn::match_and_decode(log) {
                // Burn event: (reserve0, reserve1) -= (qty0, qty1)
                let component_id = address_to_hex(log.address());
                ...
                if let Some((token0, token1)) = maybe_get_pool_tokens(&store, &component_id) {
                    deltas.extend_from_slice(&[
                        BalanceDelta {
                            ...
                            tx: Some(log.receipt.transaction.into()),
                            token: token0.clone(),
                            delta: event.delta_qty0.to_signed_bytes_be(),
                            component_id: string_to_bytes(&component_id),
                        },
                        BalanceDelta {
                            ord: log.ordinal(),
                            tx: Some(log.receipt.transaction.into()),
                            token: token1.clone(),
                            delta: event.delta_qty1.to_signed_bytes_be(),
                            component_id: string_to_bytes(&component_id),
                        },
                    ]);
                }
