...
fn create_component(factory_address: &[u8], pool: CowPool) -> Option<ProtocolComponent> {
    Some(
        ProtocolComponent::new(&pool.address.to_hex())
            .with_tokens(&[
                pool.token_a.as_slice(),
                pool.token_b.as_slice(),
                pool.lp_token.as_slice(),
            ])
            .with_contracts(&[factory_address, pool.address.as_slice()])
            .with_attributes(&[
