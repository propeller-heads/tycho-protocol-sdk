...
- **Exception:** Aave `StataToken` (stata/stk tokens) is not supported because its `Deposit`/`Withdraw` events can surface wrapped Aave aToken amounts instead of ERC4626 asset/share amounts, which breaks the standard assumptions used by this stream.

## Adding an ERC4626 pool

1. Add a new object to `params.json` following the existing format:
