...
abstract contract BalancerERC4626Helpers is BalancerCustomWrapHelpers {
        ...
        if (!isERC4626(sellToken) && isERC4626(buyToken)) {
            if (sellTokenFound) {
                ...
                for(uint256 i = 0; i < tokens.length; i++) {
                    ...
                    if (isERC4626(token) && IERC4626(token).asset() == sellToken) {
            ...
            if (sellTokenFound) {
                ...
                // output address is unused the UNWRAP action just takes sellToken which is indeed a share
    ...
    function prepareERC4626SellOrBuy(
        ...
        if (kind == ERC4626_SWAP_TYPE.SWAP_WRAP) { // !isERC4626(sellToken) && isERC4626(buyToken) and isERC4626(buyToken) && isERC4626(sellToken)
                steps = new IBatchRouter.SwapPathStep[](2);

                // swap: sellToken -> buyToken.asset()
                (,, steps[0]) = createERC20Path(
                    pool, IERC20(sellToken), IERC20(IERC4626(buyToken).asset()), specifiedAmount, false
                );
        ...
        else if (kind == ERC4626_SWAP_TYPE.SWAP_UNWRAP) { // isERC4626(sellToken) && !isERC4626(buyToken) and !isERC4626(buyToken) && !isERC4626(sellToken)
            steps = new IBatchRouter.SwapPathStep[](2);

                // swap: sellToken -> buyToken.shares()
                (,, steps[0]) = createERC20Path(
                    pool, IERC20(sellToken), IERC20(outputAddress), specifiedAmount, false
                );
        ...
        else if (kind == ERC4626_SWAP_TYPE.WRAP_SWAP) { // input is ERC20, output is ERC4626
            steps = new IBatchRouter.SwapPathStep[](2);

            // wrap: sellToken.shares() -> sellToken.asset()
            (,, steps[0]) = createWrapOrUnwrapPath(
                outputAddress, specifiedAmount, IVault.WrappingDirection.WRAP, false
            );

            // swap: sellToken.asset() -> buyToken
            (,, steps[1]) = createERC20Path(
                pool, IERC20(outputAddress), IERC20(buyToken), specifiedAmount, false
            );
        }
            ...
            (,, steps[0]) = createWrapOrUnwrapPath(
                sellToken, specifiedAmount, IVault.WrappingDirection.UNWRAP, false
            );

            // swap: sellToken.asset() -> buyToken
            (,, steps[1]) = createERC20Path(
                pool, IERC20(sellToken), IERC20(buyToken), specifiedAmount, false
            );
        }
    ...
    function swapERC4626AndERC20(
        ...
        if(!isBuy) {
            ...
            (,, uint256[] memory amountsOut) =
                router.swapExactIn(sellPath, type(uint256).max, false, bytes(""));
            ...
            router.swapExactOut(buyPath, type(uint256).max, false, bytes(""));
    ...
    function getLimitsERC4626AndERC20(
        ...
        if(kind == ERC4626_SWAP_TYPE.SWAP_WRAP) {
            ...
            limits[0] = balancesRaw[0];
            limits[1] = IERC4626(buyToken).balanceOf(pool);
        }
        else if(kind == ERC4626_SWAP_TYPE.SWAP_UNWRAP) {
            for (uint256 i = 0; i < tokens.length; i++) {
                address token = address(tokens[i]);
            }
            limits[0] = IERC4626(sellToken).balanceOf(pool);
            limits[1] = balancesRaw[0];
        }
