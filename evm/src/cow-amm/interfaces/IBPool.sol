...
interface IBPool is IERC20 {
     ...
     */
    event LOG_JOIN(
        address indexed caller, address indexed tokenIn, uint256 tokenAmountIn
    );
     ...
     */
    event LOG_EXIT(
        address indexed caller, address indexed tokenOut, uint256 tokenAmountOut
    );
     ...
     */
    event LOG_CALL(
        bytes4 indexed sig, address indexed caller, bytes data
    ) anonymous;
    ...
    /**
     * @notice Thrown when the ratio between the pool token amount and the total
     * supply is zero
     */
    ...
    /**
     * @notice Thrown when the token amount in is above maximum amount in
    ...
    /**
     * @notice Thrown when the token amount out is below minimum amount out
     * allowed by the caller
     */
    ...
    /**
     * @notice Thrown when the token amount in surpasses the maximum in ratio
     * allowed by the pool
     */
    error BPool_TokenAmountInAboveMaxRatio();

    /**
     * @notice Thrown when the spot price before or after the swap is above the
     * max allowed by the caller
     */
    error BPool_SpotPriceAboveMaxPrice();

    /**
     * @notice Thrown when the token amount out is below the minimum out allowed
     * by the caller
     */
    error BPool_TokenAmountOutBelowMinOut();

    /**
     * @notice Thrown when the spot price after the swap is below the spot price
     * before the swap
     */
    error BPool_SpotPriceAfterBelowSpotPriceBefore();

    /**
     * @notice Thrown when the spot price before the swap is above the ratio
     * between the two tokens in the pool
     */
    error BPool_SpotPriceBeforeAboveTokenRatio();

    /**
     * @notice Thrown when the token amount out surpasses the maximum out
     * allowed by the pool
     */
    error BPool_TokenAmountOutAboveMaxOut();

    /**
     * @notice Thrown when the pool token amount out is below the minimum pool
     * token amount out allowed by the caller
     */
    ...
    /**
     * @notice Thrown when the pool token amount in is above the maximum amount
     * in allowed by the caller
     */
    ...
    /**
     * @notice Joins a pool, providing each token in the pool with a
     * proportional amount
    ...
    /**
     * @notice Exits a pool, receiving each token in the pool with a
     * proportional amount
     * @param poolAmountIn The amount of pool tokens to burn
     * @param minAmountsOut The minimum amount of tokens to receive from the
     * pool
     */
    ...
    /**
     * @notice Swaps as many tokens in as needed for an exact amount of tokens
     * out
     * @param tokenIn The address of the token to swap in
    ...
    /**
     * @notice Gets the spot price of tokenIn in terms of tokenOut without the
     * fee
     * @param tokenIn The address of the token to swap in
     * @param tokenOut The address of the token to swap out
    ...
    /**
     * @notice Gets the current array of tokens in the pool, while the pool is
     * not finalized
    ...
    function getTotalDenormalizedWeight()
        external
        view
        returns (uint256 totalDenormWeight);
