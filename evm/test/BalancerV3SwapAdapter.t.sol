...
contract BalancerV3SwapAdapterTest is AdapterTest, ERC20, BalancerV3Errors {
    ...
    function setUp() public {
        ...
        deal(ERC20_GOETH, address(this), 1000 * 10**18);
        ...
        // Approve tokens for Buffer Router through Permit2
        ...
        // Approve ERC4626 tokens for Buffer Router
        ...
        IERC20(address(ERC4626_sGOETH)).approve(permit2, type(uint256).max);
        IPermit2(permit2).approve(
        ...
        // Approve Permit2 to spend ERC4626 for the Balancer vault
        IPermit2(permit2).approve(
            address(ERC4626_sUSDC),
        ...
        // Initialize Balancer's internal ERC4626 buffer through the Buffer Router
        IBufferRouter bufferRouter = IBufferRouter(0x9179C06629ef7f17Cb5759F501D89997FE0E7b45);
        ...
        ERC4626_sGOETH.deposit(10 * 10**18, address(this));
        ...
        bufferRouter.initializeBuffer(
            ...
            10 * 10**18,                // exactAmountUnderlyingIn (10 ERC20_GOETH)
            10 * 10**18,                // exactAmountWrappedIn (10 ERC4626_sGOETH)
        ...
        // Deal ERC20 to test contract
        deal(ERC20_USDC, address(this), 1000000 * 10**6);
        deal(ERC20_GOETH, address(this), 1000 * 10**18);
        deal(ERC20_ETHx, address(this), 1000 * 10**18);
        
        // Approve ERC20 spending to ERC4626 vault
        IERC20(ERC20_USDC).approve(address(ERC4626_sUSDC), type(uint256).max);
        IERC20(ERC20_GOETH).approve(address(ERC4626_sGOETH), type(uint256).max);
        IERC20(ERC20_ETHx).approve(address(ERC4626_sETHx), type(uint256).max);
        
        // Deposit ERC20 to get ERC4626
        ERC4626_sUSDC.deposit(1000000 * 10**6, address(this));
        ERC4626_sGOETH.deposit(1000 * 10**18, address(this));
        ERC4626_sETHx.deposit(1000 * 10**18, address(this));
        

        IERC20(ERC20_USDC).approve(address(balancerV3Vault), type(uint256).max);
        IERC20(ERC20_GOETH).approve(address(balancerV3Vault), type(uint256).max);
    ...
    function testPriceFuzzBalancerV3_ERC4626_ERC20_DIRECT(
        uint256 amount0
    ) public {
    ...
    function testPriceFuzzBalancerV3_ERC20_ERC20_DIRECT(
        uint256 amount0
    ) public {
    ...
    function testSwapFuzzBalancerV3_ERC4626_ERC20_ERC20_UNWRAP_SWAP(
        ...
        if (isBuy) {
            vm.assume(
                specifiedAmount < limits[1] &&
                    specifiedAmount > 10**18 // as using a mock USDC, we set a custom limit here.
            );
        } else {
            vm.assume(
                specifiedAmount < limits[0] &&
    ...
    function testPriceFuzzBalancerV3_ERC4626_ERC4626_DIRECT(
        ...
        bytes32 pool = bytes32(
            bytes20(ERC4626_ERC4626_WETH_wstETH_STABLE_POOL)
        );
    ...
    ///////////////////////////////////////// ERC4626-->ERC20-->ERC4626 UNWRAP_SWAP
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    ...
    function testSwapFuzzBalancerV3_ERC4626_ERC20_ERC4626_UNWRAP_SWAP(
        ...
        if (side == OrderSide.Buy) {
            ...
            ///////////////////////////////////////// ERC20-->ERC4626-->ERC4626-->ERC20 UNWRAP_SWAP_WRAP
    //////////////////////////////////////////////////////////////////////////////////////////////////////////////////

    function testPriceFuzzBalancerV3_ERC20_ERC4626_ERC4626_ERC20_UNWRAP_SWAP_WRAP(uint256 amount0)
        public
    {
        address token0 = address(ERC4626_sUSDC);
        address token1 = address(ERC4626_sGOETH);

        bytes32 pool = bytes32(bytes20(ERC20_ERC20_GOETH_USDC_WEIGHTED_POOL));
        uint256[] memory limits = adapter.getLimits(pool, token0, token1);

        // Use bounds appropriate for sUSDC (6 decimals)
        amount0 = bound(amount0, 10 * 10**6, 100000 * 10**6);
        vm.assume(amount0 < limits[0]);
    ...
    function testSwapFuzzBalancerV3_ERC20_ERC4626_ERC4626_ERC20_UNWRAP_SWAP_WRAP(
        ...
        // Scale bounds based on whether it's a buy or sell order
        if (isBuy) {
            // For buy orders, bound sGOETH amount (18 decimals)
            specifiedAmount = bound(
                specifiedAmount,
                10**17,
                1000 * 10**18
            ...
            // For sell orders, bound sUSDC amount (6 decimals)
            specifiedAmount = bound(
                specifiedAmount,
                10 * 10**6,
                100000 * 10**6
